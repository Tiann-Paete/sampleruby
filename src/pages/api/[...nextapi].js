  ...
  try {
    ...
    for (const item of items) {
      const [priceInfo] = await connection.query(
        `SELECT productid FROM aproduct_prices WHERE priceid = ?`,
        [item.priceid]
      );
    ...
    const [orderResult] = await connection.query(
      ...
      [
        tracking_number, 
        customerid,
        baseProductId || 0,  // Use the actual productid from aproducts
        totalQuantity,       // Total quantity for this product
        total_amount, 
        payment_method, 
        delivery_address,
        'order_placed'
